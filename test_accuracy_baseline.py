...
for sequence_name in sequences:
    ...
    for cameras in cameras_list:
        ...
        # Load dut file
        DUT = {}
        with open(os.path.join(data_path,sequence_name+"."+str(cameras[0])+".json" ), "r") as f:
            file = json.load(f)
        for frame in file:
        ...
        thread_id = 0
        while step <= step_max:
            proc = Process(target=hota_base_par, args=(res,thread_id,predicted_keypoints, ground_truth_keypoints, step))
            procs.append(proc)
            proc.start()
            step += step_size
            thread_id += 1
    ...
    df.to_csv(os.path.join(result_path,date+"_"+sequence_name+'_V'+version+'cam.csv'), index=False)  
    df_debug.to_csv(os.path.join(result_path,date+"_"+sequence_name+'_V'+version+'cam_debug.csv'), index=False)